...
our $EXTENDED_ATAC_DONOR_CONSENSUS_VALIDATION = 0; # turn on if you want more strict behavior
...
sub new {
    ...
    my $self = {
        ...
        cdna_acc => undef, # same as acc above, treat as alias
        align_acc => undef, # name of the alignment for that cdna 
        cdna_id => undef, ## refers to a specific transcript sequence
        align_id => undef,  ## refers to a specific alignment of the transcript sequence. (single cdna_ids can have multiple align_ids)
        ...
        prog => undef, # indicate alignment program responsible
...
sub get_cdna_id {
    ...
    my $cdna_id = $self->{cdna_id};
    unless (defined $cdna_id) {
        confess "Error, cdna_id not set for alignment " . $self->toString();
    }
...
sub get_align_id {
    ...
    if (! defined $align_id) {
        confess "Error, align_id not set for alignment " . $self->toString();
    }
            ...
            if ($left_chars eq $splice_chars_left && $right_chars eq $splice_chars_right) {
                ...
                if ($ALLOW_ATAC_splice_pairs) {
                    if ( ( $orient eq '+' && $left_chars eq 'AT')
                        ...
                        # now making this optional, as of 12-19-2013 bhaas
                        if ($EXTENDED_ATAC_DONOR_CONSENSUS_VALIDATION && !&_validates_AT_AC_donor_extended_consensus($orient, $intron_lend, $intron_rend, $sequence_ref)) {
                            next CONSENSUS_PAIR;
                        }
                    }
                }
...
sub toString() {
    my $self = shift;
    my $text = "\n\n"
        . "Scaffold: " . $self->{genome_acc} . "\n"
        . "Trans acc: " . $self->get_acc() . "\n"
...
sub set_acc () {
    ...
    my $acc = shift;
    $self->{acc} = $acc;
    $self->{cdna_acc} = $acc; # same as acc, but different from align_acc
}
